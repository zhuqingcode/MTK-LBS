...
#if 0
u8 Queue_ServUpdatabufindex[UPDATA_BUFNUM]; //posbuf写入队列标示，0为占用，1使用中
...
//	OSSemPend(JT808_PrtclHandPrtctSem, 0, &err);
	switch(type)
	{
		...
		break;
		case e_CallcarOrder:
		//	if(buflen-50>sizeof(sProtclHandl.stCallcarOrder.OrderContent)) //数据过程未处理 ？？
		//		return 1;
		...
		break;
		case e_CallcarInfoOrder:
		//	if(buflen-50-20>sizeof(sProtclHandl.stCallcarOrder.OrderContent)) //数据过程未处理 ？？
		//		return 1;
			Mem_Copy(sProtclHandl.stCallcarOrder.OrderID,pbuf,50);
...
*				UpdateOnly只更新ram的LbsCfgStruct
*				UpdateFlash 更新ram同时写入flash
*				写flash：可以在参数设置的同时写flash，也可以在CfgPara为eCfaParaMax时直接写flash
*input:			CfgPara 参数对应的JTT808协议ID
*				pValue要写入的值，len写入长度
*				UpdateMode UpdateOnly只更新ram全局变量，UpdateFlash同时写flash				
*Return:		ActionOK表成功，ParaError参数错误
*Others:		
---------------------------------------------------------------------------------------*/
//判断len长度字符串是否为IP IP格式xxx.xxx.xxx.xxx peijl_20120828 修改
...
#if 0
		case eOnlineUpgrad_Ctrl:	//无线升级
		{

			step=0;
			while(len<(msgbodylen-1)) //取升级参数
			{
				p = Str_Str(pmsgbody, ";");
				if(p)
				{
			...
			SetUpgradStarttimeS(); //peijl_20121029 添加：设置升级计时开始时间
			}
		}
		break;

		case eSpclServ_Ctrl:	//控制终端连接指定服务器
		{
			if(*pmsgbody==1) //切换回主监考平台
			{
				gServLimittim=0; //无时限
