...
{	
	...
	{
		ret = SaveAreaData(pbuf,Circular_Area,option,&read_len);  //±£´æ
...
static u8 set_rect_area(u8 *pbuf, u16 buflen)
{
	...
	for(i=0;i<area_num;i++)
	{
		ret = SaveAreaData(pbuf,Rectangle_Area,option,&read_len);  //±£´æ
		if(option == 0)	 //Èç¹ûÊÇ¸üÐÂ²Ù×÷£¬Ö»ÔÚµÚÒ»´ÎÇå³ý£¬ºóÃæÔò¸ÄÎª×·¼Ó
		{
			option = 1;//
		}
...
static u8 set_poly_area(u8 *pbuf, u16 buflen)	
{
	...
	for(i=0;i<area_num;i++)
	{
		ret = SaveAreaData(pbuf,Poly_Area,option,&read_len);  //±£´æ
		if(option == 0)	 //Èç¹ûÊÇ¸üÐÂ²Ù×÷£¬Ö»ÔÚµÚÒ»´ÎÇå³ý£¬ºóÃæÔò¸ÄÎª×·¼Ó
		{
			option = 1;//
		}
...
oa_bool DevReq2ServPackag_build_blind(u16 ReqMsgId) //¼´Ê±ÉÏ´«Êý¾Ý
{
	u8 pbuf[DATA_MAX_LEN] = 0;
	u16 U16Temp = 0;
	oa_bool ret;
